    ...
    public void run() {
        for (int i = 0; i < numOfReqs; i++) {
            ...
            while (triedCount < RETRIES) {
                try {
                    long timestampAtSend = System.currentTimeMillis();
                    ...
                    long timestampAtReceive = System.currentTimeMillis();
                    ThreadServices.addToRecords(
                            new Record(timestampAtSend, "POST",
                                    timestampAtReceive - timestampAtSend, res.getStatusCode())
                    );
                    break;
