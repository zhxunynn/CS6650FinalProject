...
public class ClientMain {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        try {
            String filename =  "homework1_client2_" + ArgCommand.numOfThreads + "_" + ArgCommand.numOfRequests + ".csv";
            ThreadServices.printRecords(Paths.get(ArgCommand.filepath, filename));
        } catch (IOException e) {
            logger.severe(e.getMessage());
            System.exit(-1);
        }
    ...
    private static void doLatency() throws InterruptedException {
        ...
        testLatch.await();
        long endTime = System.currentTimeMillis();
        logger.info("Total Duration is " + 1.0 * (endTime - startTime) / 1000 + "s with average latency about " + 1.0 * (endTime - startTime) / ArgCommand.numOfRequests + " s.");
    }
